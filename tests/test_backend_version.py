...
def test_version_endpoint_reports_build(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv('STLTOSOLID_DATA', str(tmp_path))
    monkeypatch.setenv('STLTOSOLID_BUILD_SHA', 'abc1234')
    monkeypatch.setenv('STLTOSOLID_BUILD_TIME', '2026-08-18T03:00Z')
...
def test_version_endpoint_defaults_to_unknown(monkeypatch, tmp_path):
    from fastapi.testclient import TestClient
    monkeypatch.setenv('STLTOSOLID_DATA', str(tmp_path))
    monkeypatch.delenv('STLTOSOLID_BUILD_SHA', raising=False)
    monkeypatch.delenv('STLTOSOLID_BUILD_TIME', raising=False)
