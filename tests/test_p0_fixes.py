...
def test_hole_end_exits_probe():
    from stl_to_solid.features import hole_end_exits
...
def test_dominant_axis_offers_snapped_and_raw():
    from stl_to_solid.extrusion import dominant_axis
...
def test_weld_does_not_merge_distinct_vertices():
    from stl_to_solid.mesh_prep import weld_vertices
...
def test_shape_constancy_rejects_drift():
    from stl_to_solid.extrusion import score_axis
    m = trimesh.load(synth.export(synth.rotate(synth.plate_holes(), (1, 0, 0), 3.0),
...
def test_dense_cad_export_is_not_a_scan(tmp_path):
    from stl_to_solid.mesh_prep import load_mesh, classify, SCAN_MIN_FACES
...
def test_jittered_dense_mesh_is_a_scan(tmp_path):
    from stl_to_solid.mesh_prep import load_mesh, classify, mean_dihedral_deg, coplanar_fraction
...
def test_real_scans_with_some_coplanar_pairs_are_scans():
    ...
    from stl_to_solid.mesh_prep import (SCAN_MAX_COPLANAR_FRAC, SCAN_DIHEDRAL_DEG,
                                     SCAN_MIN_FACES)
...
def _fake_scan_worker(src, dst, target_faces, method, arg):
    ...
    with open(os.environ['STLTOSOLID_FAKE_LOG'], 'a') as fh:
...
def test_scan_ladder_pre_reduces_once(tmp_path, monkeypatch):
    from stl_to_solid.mesh_prep import _poisson_rebuild
    log = tmp_path / 'log.txt'
    monkeypatch.setenv('STLTOSOLID_FAKE_LOG', str(log))
