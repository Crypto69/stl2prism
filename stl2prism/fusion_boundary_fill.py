...
generated from stl2prism's segmentation.
...
def run(context):
    ...
    try:
        ...
        if design is None:
            ui.messageBox('stl2prism: open a design first')
            return
        ...
        text = '\n'.join(log)
        try:
            app.log('stl2prism boundary fill\n' + text)
        except Exception:
            pass
        if not made:
            ui.messageBox('stl2prism: no solid was built\n\n' + text)
        elif SHOW_SUMMARY:
            ui.messageBox('stl2prism: {} solid(s) built\n\n{}'.format(len(made), text))
    except:
        if ui:
